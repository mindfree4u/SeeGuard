...
import { View, Text, Image, TextInput, TouchableOpacity, Alert, Modal, StyleSheet, ScrollView } from 'react-native';
import CheckBox from '@react-native-community/checkbox';
import { useRoute } from '@react-navigation/native';
import styles from './styles/Intro_21000styles';
import {AuthenticationContext } from '../App';

const Intro_21000 = ({ navigation, route }) => {
 ...
 useEffect(() => {
       if (route.params && route.params.phone) {
        ...
        // setInitialPhone(route.params.phone);   인증완료 후 폰번호 변경시
       }
     }, [route.params]);
  ...
  const handleSignUp = () => {
      if (!Authentication) {
              Alert.alert('', '휴대폰으로 본인인증을 해주세요.', [{ text: '확인' }]);
            }
      else if (!email) {
              Alert.alert('', '이메일 주소를 입력하세요.', [{ text: '확인' }]);
            }
      ...
      else if(isPasswordValid && agree1 && agree2 && Authentication) {
        navigation.navigate('Intro_21200');
      }
    };
  ...
  return (
      <ScrollView>
        <View style={styles.root}>
            ...
            <Text style={[styles.text1, { left: 40, top: 132 }]}>아래 내용을 입력해주세요.</Text>
             { Authentication ? (
            // <View style={{ position: 'absolute', left: 35, top: 160 , flexDirection: 'row', alignItems: 'center'}}>
               <>
                    <Text style={[styles.text1, { left: 40, top: 160 }]}>휴대폰 번호</Text>
                    <Text style={[styles.textinput, { left: 40, top: 185 }]}>{phone}</Text>
                   <View style={[styles.line, {top: 210, width: '50%'}]} />
                   <TouchableOpacity style={[styles.Rectangle4, { top: 170, Rightmargin: '5%' }]}>
                     <Text style={styles.본인인증}>인증완료 V</Text>
                   </TouchableOpacity>
             </>
                )                             :
            (
            <TouchableOpacity style={styles.Component2} onPress={() => navigation.navigate('Intro_21100')}>
                <Text style={styles.본인인증}>휴대폰으로 본인인증</Text>
            </TouchableOpacity>)
            }
            <Text style={[styles.text1, { left: 40, top: 253 }]}>이메일주소</Text>
            <TextInput style={[styles.textinput, {width: '50%'}]} value={email} maxLength={100}
            onChangeText={ handleEmail} placeholder="abcd@abcd.com" placeholderTextColor="#D9D9D9"/>
            <View style={[styles.line, {width: '50%'}]} />
            <TouchableOpacity style={styles.Rectangle4} onPress= { duplicationCheck }>
                <Text >{ isEmailAvailable ? '확인' : '중복체크'}</Text>
            </TouchableOpacity>
            {isEmailAvailable ? (
                    <Text style={styles.text5}>*사용 가능한 이메일입니다.</Text>
                  ) : null}
            <Text style={[styles.text1, { left: 40, top: 353 }]}>비밀번호</Text>
            <TextInput  value={password} style={[styles.textinput, { left: 40, top: 366, width: '80%' }]}
            ...
            <View style={[styles.line, { left: 40, right:41, top: 403 }]} />
            <Text style={[styles.text1, { left: 40, top: 433 }]}>비밀번호 확인</Text>
            <TextInput value={confirmPassword} style={[styles.textinput, { left: 40, top: 446, width: '80%' }]}
            ...
            <View style={[styles.line, { left: 40, right:41, top: 483 }]} />
            {isPasswordValid ? ( <Text style={[styles.text5, { top: 491 }]}>*사용 가능한 비밀번호입니다.</Text> )
                                :null }
            { !isPasswordValid  && password && confirmPassword ? ( <Text style={[styles.text5, { top: 491, color: '#FA584E' }]}>
            ...
            <CheckBox style={{position: 'absolute', left: 42, top: 615,}} tintColors={{ true: '#4D4D4D' }}
